...
Requests API transport that uses NSURLSession and friends via PyObjC 2.1 that
ships with macOS.

Much of this code was copied, rewritten, or inspired by gurl.py from the munki
project.
...
from Foundation import (  # NSOperationQueue,; NSURLAuthenticationMethodServerTrust,; NSURLRequestReturnCacheDataElseLoad,; NSURLRequestUseProtocolCachePolicy,
    NSURL,
    NSBundle,
    NSDate,
    NSMutableData,
...
def build_request(request, timeout):
    """Convert a requests `PreparedRequest` into a suitable
    NSMutableURLRequest."""
...
def build_response(req, delegate, cookiestore):
    """Build a requests `Response` object from the response data collected by
    the NSURLSessionDelegate, and the default cookie store."""
    ...
    # Add new cookies from the server. For NSURLSession these have already
    # been parsed.
...
class NSURLCredentialAuth(AuthBase):
    ...
    It isn't possible to use the requests built-in HTTPBasicAuth adapter
    because it adds headers, which doesn't really gel with NSURLSession's
    delegate challenge methods."""
    ...
    def __call__(self, r):
        """Instead of modifying the request object, we construct an instance of
        NSURLCredential to attach to ourselves.

        When the delegate detects that attribute is present, it uses it
        whenever a challenge comes in."""
...
class NSURLSessionAdapterDelegate(NSObject):
    """NSURLSessionAdapterDelegate implements the delegate methods of
    NSURLSession protocols such as:
    ...
    https://developer.apple.com/documentation/foundation/
        nsurlsessiontaskdelegate?language=objc
    - NSURLSessionDataDelegate
    https://developer.apple.com/documentation/foundation/
        nsurlsessiondatadelegate?language=objc>
    """
    ...
    def URLSession_task_didReceiveChallenge_completionHandler_(
        ...
        logger.debug(
            "NSURLProtectionSpace host: %s, realm: %s, method: %s",
            host,
            realm,
    ...
    def URLSession_task_HTTPRedirection_newRequest_completionHandler(
...
class NSURLSessionAdapter(BaseAdapter):
    def __init__(self, credential=None, force_basic=False):
        """NSURLSessionAdapter implements a requests adapter using PyObjC +
        NSURLSession.

        Because of the way the delegate deals with authentication, we cannot
        support requests style auth which may only act on the content of the
        request. For NSURLSessionAdapter you should use NSURLCredentialAuth,
        which supplies an NSURLCredential object whenever authentication is
        required by the remote host.

        :param credential: NSURLCredential that will be used if the server
            requests basic auth.
        :param force_basic: Force basic authentication to be used for every
            request by preparing the Authorization
            header. This is sometimes required because NSURLSession only checks
            with the delegate for credentials when the server sends a
            "challenge". Some web services never even send a challenge.
        """
    ...
    def send(
        ...
        self.session = NSURLSession.sessionWithConfiguration_delegate_delegateQueue_(
            self.configuration, self.delegate, None,
        )
        ...
        if request.method in ["PUT", "POST"] and request.body is not None:
            # These verbs should usually be an upload task to send the correct
            # request headers.
        ...
        if self.delegate.SSLError is not None:
            ...
            raise SSLError(
                "{}: {}".format(code, message), response=response, request=request
            )
