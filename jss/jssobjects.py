...
class Computer(Container):
    ...
    # - It does now, at least in 10.5 -mo
...
class ComputerExtensionAttribute(Container):
    ...
    data_keys = {
        ...
        "data_type": None,
        "input_type": {
            "type": None,  # script, Pop-up Menu, LDAP Attribute Mapping,
            # Text Field
            "script": None,
            "popup_choices": None,  # Array of <choice>value</choice>
            # for Pop-Up type
        },
...
class OSXConfigurationProfile(Container):
    ...
    def add_payloads(self, payloads_contents):
        """Add xml configuration profile to the correct tag in the
        OSXConfigurationProfile object.
        ...
        # Normally, when embedding xml within xml you would think that a normal
        # person would require escaping to avoid parsing issues. But nope, we
        # just put the profile inline with the payloads tag.
...
class Policy(Container):
    ...
    def get_scripts(self, priority=None):
        ...
        Args:
            priority (str, optional): One of "After" or "Before" to filter
                attached scripts by their priority.
        """
