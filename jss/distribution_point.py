...
try:
    ...
    # removed OrdinaryCallingFormat as not used
    from boto.s3.key import Key

    BOTO_AVAILABLE = True
except ImportError:
    print(
        "boto is not available, you will not be able to use the AWS"
...
class Repository(object):
    ...
    Attributes:
        ...
        required_attrs (Set): A set of the keys which must be supplied to the
            initializer, otherwise a JSSError will be raised.
...
class DistributionServer(Repository):
    ...
    def _build_url(self):
        ...
        This actually still works in some scenarios, but it seems like it will
        be deprecated soon.
        """
    ...
    def _build_url_modern(self):
        ...
        This uses the UploadServlet that has been used to handle most file
        uploads into JAMF Pro.
        """
        self.connection["upload_url"] = "%s/%s" % (
            self.connection["jss"].base_url,
            "upload",
    ...
    def _copy_new(self, filename, id_=-1, file_type=0):
        ...
        resource = open(filename, "rb")
        headers = {
            "sessionIdentifier": "com.jamfsoftware.jss.objects.packages."
            "Package:%s" % str(id_),
...
class CloudDistributionServer(Repository):
    ...
    def package_index_using_casper(self):
        ...
        It will test for whether the file exists on only cloud distribution
        points.
        """
        casper_results = casper.Casper(self.connection["jss"])
...
def _jcds_upload_chunk(
    ...
    Args:
        ...
        base_url (str): The JCDS base URL which includes the regional hostname
            and the tenant id.
...
class AWS(CloudDistributionServer, abstract.AbstractRepository):
    """Class for representing an AWS Cloud Distribution Point and its
    controlling JSS.
    """
    ...
    def __init__(self, **connection_args):
        ...
        It is more secure to use the following environment variables provided
        by boto:

            AWS_ACCESS_KEY_ID - The access key id to the jamf bucket
            AWS_SECRET_ACCESS_KEY - The secret access key to the jamf bucket

        You may also use the file ~/.boto as described in the boto
        documentation.
        ...
        Args:
            connection_args: Dict, with required keys:
                ...
                secret_access_key (optional): The secret access key, use
                    environment instead.
                host (optional): A bucket host. Seems to be needed if your
                    bucket is not in the default location
        ...
        self.chunk_size = connection_args.get("chunk_size", 52428800)
        # 50 mb default
    ...
    def exists(self, filename):  # type: (str) -> bool
        """Check whether a package already exists by checking for a bucket item
        with the same filename.

        Args:
            filename: full path to filename. Only the name itself will be
                checked.
...
class JCDS(CloudDistributionServer):
    ...
    The JSS allows direct upload to the JCDS by exposing the access token from
    the package upload page.
    ...
    def _scrape_tokens(self):
        """Scrape JCDS upload URL and upload access token from the jamfcloud
        instance."""
        ...
        if matches is None:
            raise JSSError(
                "Did not find the JCDS base URL on the packages page. Is this "
                "actually Jamfcloud?"
            )
        ...
        if matches is None:
            raise JSSError(
                "Did not find the JCDS upload token on the packages page. Is "
                "this actually Jamfcloud?"
            )
        ...
        # This is to make JSSImporter happy because it accesses .connection
        self.connection["url"] = jcds_base_url
    ...
    def _copy_multiprocess(self, filename, upload_token, id_=-1):
        """Upload a file to the distribution server using multiple processes
        to upload several chunks in parallel.
        ...
        for chunk in xrange(0, total_chunks):
            ...
            print(
                "id: {0}, version: {1}, size: {2}, filename: {3}, "
                "lastModified: {4}, created: {5}".format(
                    data["id"],
                    data["version"],
                    data["size"],
                    data["filename"],
    ...
    def _copy_threaded(self, filename, upload_token, id_=-1):
        """Upload a file to the distribution server using multiple threads to
        upload several chunks in parallel."""
        fsize = os.stat(filename).st_size
        total_chunks = int(math.ceil(fsize / JCDS.chunk_size))
    ...
    def _copy_sequential(self, filename, upload_token, id_=-1):
        """Upload a file to the distribution server using the same process as
        python-jss.
    ...
    def _copy(self, filename, id_=-1, file_type=0):
        ...
        # else:
        #     self._copy_threaded(
        #                       filename, self.connection['jcds_upload_token'])
