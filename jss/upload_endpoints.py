...
class Upload(object):
    ...
    - A file identifier to indicate what type of file is being uploaded. This
        is a number corresponding to some kind of internal enum.
    - A session token, which is used as a CSRF token for each upload.
    - Request data is posted to
        /upload/?sessionIdentifier=&fileIdentifier=&session-token=
    - The response is an empty body (200).
    - The ajax call to monitorUploadProgress.ajax receives a redirect URL when
    ...
    def monitor_progress(self):
        """The JAMF Pro Web UI often calls this endpoint to get upload status
        (to display a progress bar). The secondary functionality is that when
        the upload is finished, the payload may contain a redirect URI. For
        this reason we still need to use the endpoint to determine the upload
        outcome.

        The format of the response is an XML fragment, even though no
    ...
    def finalize(self):
        """POST to the legacy.html page representing the object, which confirms
        that the upload is complete.

        For this, it seems we need some of the hidden vars from the FORM
        previous:

        - session-token
        - INCLUDE_PAGE_VARIABLE (the server uses this to determine the previous
            step, and therefore what the current step requires).
        - OBJECT_RANDOM_IDENTIFIER (matches the identifier appended to the end
            of session_identifier)
        """
        pass
...
class UploadAssistant(object):
    ...
    Although uploads are tracked by the
    `com.jamfsoftware.jss.fileupload.UploadController` there are sometimes other
...
class PushNotificationKeystoreAssistant(UploadAssistant):
    ...
    def __init__(self, j, **kwargs):
        super(UploadAssistant, self).__init__(
            j,
            [
...
class MacOSConfigurationProfileUpload(Upload):
    """This object represents a direct upload of a macOS configuration profile
    to the web ui.
...
class PushNotificationKeystoreUpload(Upload):
    def __init__(self, jss):
        """Construct a PushNotificationKeystoreUpload (Upload a .p12 file as
        push keystore).

        Notes:

        - The sessionIdentifier is constructed like so
          (see WEB-INF/frontend/standardFormUpload.jsp):

                sessionIdentifier =
                    target.getClass().getName() + ":"
                    + target.getRandomIdentifier();

          In this case the classname becomes:
          com.jamfsoftware.jss.objects.pushnotification.PushNotificationKeystore

        - The fileIdentifier is always "Keystore" (It is hardcoded into the
          .jsp, see WEB-INF/frontend/pushCertificateUploadP12.jsp).

        - The object random identifier is generated at page load time, so we
          have to scrape it at least once.

        Sequence:

        - Hit `legacy/pushNotificationCertificate.html?id=-1&o=c`, which is
          usually in an IFRAME.
        ...
        - When the monitor ajax endpoint says complete, POST again to
          legacy/pushNotificationCertificate.html with the
          INCLUDE_PAGE_VARIABLE from the previous step.
        - Now we are entering the keystore password
        - POST the keystore password as `FIELD_UPLOADED_P12_PASSWORD` and
          include the INCLUDE_PAGE_VARIABLE from the last page. with action=Next
        - Done.
    ...
    def finalize_p12_passphrase(self):
        """The upload of a .p12 keystore has a second phase of finalization
        which is to specify the passphrase.

        The passphrase is included in a form variable
        `FIELD_UPLOADED_P12_PASSWORD`, as well as the variables from
