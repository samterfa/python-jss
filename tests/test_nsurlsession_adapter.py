...
@pytest.mark.skipif(
    sys.platform.startswith("linux"), reason="PyObjC not present on linux"
)
@pytest.mark.macos
class TestNSURLSessionAdapter(object):
    ...
    def test_delete_xml(self, session, jss_prefs_dict, etree_building):
        ...
        response = session.delete(
            "{}/JSSResource/buildings/name/{}".format(
                jss_prefs_dict["jss_url"], etree_building.findtext("name")
            ),
            headers={"Content-Type": "text/xml", "Accept": "text/xml"},
            verify=False,
        )
