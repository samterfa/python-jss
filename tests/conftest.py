...
def dp_smb_ip_port(docker_ip, docker_services):
    docker_services.wait_until_responsive(
        timeout=30.0,
        pause=0.1,
        check=lambda: is_smb_responsive(
            "//jss:jss@%s:%s" % (docker_ip, docker_services.port_for("samba", 139))
        ),
    )
...
def pkg_path():
    return os.path.abspath(
        "testdata/Microsoft_Outlook_2016_16.15.18070902_Installer.pkg"
    )
...
def s3_connection():
    ...
    return boto.s3.connect_to_region(
        "ap-southeast-2", calling_format=boto.s3.connection.OrdinaryCallingFormat()
    )
